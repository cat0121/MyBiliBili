...
class ChaseViewCell: UITableViewCell {
    ...
    func layoutPageSubviews() {
        ...
        chaseTitle.snp_makeConstraints { (make) in
            make.left.equalTo(chaseImg.snp_right).offset(7)
        ...
        chaseNew.snp_makeConstraints { (make) in
            make.left.equalTo(chaseImg.snp_right).offset(7)
            make.top.equalTo(chaseWatch.snp_bottom).offset(2)
            make.height.equalTo(20)
        }
    }
    ...
    //数据源
    var _chaseDetails: ChaseDetails!
    var chaseDetails: ChaseDetails {
        get {
            return _chaseDetails
        }
        set {
            _chaseDetails = newValue
            _chaseImg.sd_setImageWithURL(NSURL(string: newValue.cover) ,placeholderImage: UIImage(named: "default_img"))
            _chaseTitle.text = newValue.title
            _chaseWatch.text = String.init(format: "看到第%@话", newValue.last_ep_index!)
            _chaseNew.text = String.init(format: "更新至第%@话", newValue.newest_ep_index!)
        }
    }
}